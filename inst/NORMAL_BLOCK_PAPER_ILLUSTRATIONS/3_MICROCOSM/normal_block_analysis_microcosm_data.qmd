...
```{r running-zimvnb}
zimvnb_all <- normal_block(data, blocks = 2:8, zero_inflation = TRUE,
                       sparsity = 0.005,
                       control = NB_control(verbose = TRUE))
zimvnb <- zimvnb_all$get_best_model("BIC")
```
...
taxo_reference$Phylum  <- as.integer(as.factor(taxo_reference$Phylum))
taxo_reference$Class   <- as.integer(as.factor(taxo_reference$Class))
taxo_reference$Order   <- as.integer(as.factor(taxo_reference$Order))
taxo_reference$Family  <- as.integer(as.factor(taxo_reference$Family))
taxo_reference$Genus   <- as.integer(as.factor(taxo_reference$Genus))
taxo_reference$Species <- as.integer(as.factor(taxo_reference$Species))
