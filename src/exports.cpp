...
template <typename Model>
Rcpp::List zi_known_clusters_result(const Model& model) {
  return Rcpp::List::create(
    Rcpp::Named("B")         = model.B(),
    Rcpp::Named("dm1")       = to_rvector(model.dm1()),
    Rcpp::Named("Omegaq")    = model.Omegaq(),
...
template <typename Model>
Rcpp::List zi_unknown_clusters_result(const Model& model) {
  return Rcpp::List::create(
    Rcpp::Named("B")         = model.B(),
    Rcpp::Named("dm1")       = to_rvector(model.dm1()),
    Rcpp::Named("Omegaq")    = model.Omegaq(),
...
//'
//' Equivalent of the R6 class nb_known_clusters (R/nb_known_clusters-Class.R),
...
//' R/NB-Class.R, `sparsity_weights`); only used when sparsity > 0
...
//' @noRd
// [[Rcpp::export]]
Rcpp::List nb_known_clusters_fit(const arma::mat& Y, const arma::mat& X, const arma::mat& C,
...
//'
//' @inheritParams nb_known_clusters_fit
...
//' @noRd
// [[Rcpp::export]]
Rcpp::List nb_unknown_clusters_fit(const arma::mat& Y, const arma::mat& X,
                                    arma::mat B0, arma::vec dm1_0, arma::mat Omegaq0,
...
//'
//' Equivalent of the R6 class zi_known_clusters (R/zi_known_clusters-Class.R).
...
//' zero-inflation component (private$ZI_cond_mean in R/NB-Class.R)
//' @param C fixed cluster-indicator matrix (p x q)
//' @param B0 initial regression coefficients (d x p)
//' @param dm1_0 initial inverse variance per variable (length p)
//' @param Omegaq0 initial precision matrix of the blocks (q x q)
...
//' @noRd
// [[Rcpp::export]]
Rcpp::List zi_known_clusters_fit(const arma::mat& Y, const arma::mat& X,
  ...
  if (noise_covariance == "diagonal") {
    ...
    model.run_em(niter, threshold);
    return zi_known_clusters_result(model);
  } else if (noise_covariance == "spherical") {
...
//'
//' Equivalent of the R6 class zi_unknown_clusters (R/zi_unknown_clusters-Class.R).
...
//'
//' @inheritParams zi_known_clusters_fit
//' @param C0 initial variational membership probabilities (p x q)
//' @param alpha0 initial cluster prior probabilities (length q)
//' @param M0 initial variational mean of the cluster effects (n x q)
...
//' @noRd
// [[Rcpp::export]]
Rcpp::List zi_unknown_clusters_fit(const arma::mat& Y, const arma::mat& X,
  ...
  if (noise_covariance == "diagonal") {
    zi_norm_block_cov_diag_noise_unknown_clusters model(data, B0, dm1_0, Omegaq0, C0, alpha0, M0, S0,
                                                         sparsity, sparsity_weights, fixed_tau);
    model.run_em(niter, threshold);
    return zi_unknown_clusters_result(model);
  } else if (noise_covariance == "spherical") {
