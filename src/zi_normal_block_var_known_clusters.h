...
public:
  ...
  // C_obs Omega^{-1} C_obs^T. The same Woodbury/determinant-lemma trick
  // applies per row, through the row's own posterior precision
  // Gamma^{(i),-1} = Omega + diag((W C)_{i.}) (the matrix E_step() already
  // inverts to get Gamma_.slice(i)/Mu_.row(i) -- recomputed fresh here, see
  // the non-ZI objective() for why):
  //
  //   log|Sigma_Y,i|                 = -sum_{j obs} log(dm1_j) - log|Omega| - log|Gamma^{(i)}|
  ...
  // Omega + diag(dm1C.row(i)) add a *non-negative* diagonal to Omega (dm1
  // > 0, C is 0/1), which by Weyl's inequality can only raise -- never
  // lower -- the smallest eigenvalue relative to Omega's own. So whenever
  // the extrapolated Omega candidate itself passes state_is_feasible(),
