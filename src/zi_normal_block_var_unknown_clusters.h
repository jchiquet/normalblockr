#ifndef NORMALBLOCKR_ZI_NORMAL_BLOCK_VAR_UNKNOWN_CLUSTERS_H
#define NORMALBLOCKR_ZI_NORMAL_BLOCK_VAR_UNKNOWN_CLUSTERS_H
...
public:
  ...
  // R/ZINormalBlockVarUnknownClusters.R: same shape as the non zero-inflated criterion
  // (NormalBlockVarUnknownClusters::objective), but normalized by nY/npY
  ...
  std::unique_ptr<NormalBlockVarBase> clone() const override {
    return std::make_unique<ZINormalBlockVarUnknownClusters<NoisePolicy>>(*this);
  }
  void restore_from(const NormalBlockVarBase& other) override {
    copy_tracked_state_from(other);
    const auto& o = static_cast<const ZINormalBlockVarUnknownClusters<NoisePolicy>&>(other);
    C_ = o.C_;
    alpha_ = o.alpha_;
    M_ = o.M_;
    S_ = o.S_;
...
#endif // NORMALBLOCKR_ZI_NORMAL_BLOCK_VAR_UNKNOWN_CLUSTERS_H
