...
## in NormalBlockBase.R): the 3 user-selectable algorithms (kmeans/ward2/sbm,
## via NB_control(clustering_approx = ...)) and the hidden hclustvar fallback
## used whenever the chosen algorithm fails to produce q distinct clusters,
## previously dispersed across 4 separate private methods.
set.seed(123)
...
test_that("every user-selectable clustering_approx produces a valid q-cluster init", {
  for (approx in c("kmeans", "ward2", "sbm")) {
