...
## (R/NormalBlockBase.R: private$rescale_to_original(), B_original,
...
set.seed(1)
ex <- generate_normal_block_data(n = 60, p = 12, d = 1, q = 3)
data <- NormalBlockData$new(ex$Y, ex$X)

test_that("B_original/dm1_original undo NormalBlockData's column-wise rescaling exactly", {
  model <- NormalBlockKnownClusters$new(data, ex$parameters$C)
...
test_that("fitted is on Y's original units: X %*% B_original reproduces it exactly", {
  model <- NormalBlockKnownClusters$new(data, ex$parameters$C, control = NB_control(heuristic = TRUE))
  model$optimize()
...
test_that("with scale = FALSE, B_original/dm1_original/fitted equal the internal-scale versions", {
  ...
  model <- NormalBlockKnownClusters$new(data_noscale, ex$parameters$C, control = NB_control(heuristic = TRUE))
  model$optimize()
...
test_that("warm_start_from() still copies model_par$B (internal scale), not B_original", {
  model1 <- NormalBlockUnknownClusters$new(data, 3, control = NB_control(verbose = FALSE))
  model1$optimize(NB_control(verbose = FALSE))
  model2 <- NormalBlockUnknownClusters$new(data, 3, control = NB_control(verbose = FALSE))
