###############################################################################
## Regression test: NormalBlockBase$initialize() used to fit a zero-inflation
...
set.seed(11)
ex   <- generate_normal_block_data(n = 30, p = 8, d = 1, q = 2)
data <- NormalBlockData$new(ex$Y, ex$X)

test_that("non zero-inflated models skip the kappa/B0 fit", {
  model <- NormalBlockKnownClusters$new(data, ex$parameters$C, control = NB_control(verbose = FALSE))
  ...
  model2 <- NormalBlockUnknownClusters$new(data, 2, control = NB_control(verbose = FALSE))
...
test_that("zero-inflated models still fit kappa/B0 as before", {
  model <- ZINormalBlockKnownClusters$new(data, ex$parameters$C, control = NB_control(verbose = FALSE))
  priv  <- model$.__enclos_env__$private
  expect_false(anyNA(priv$kappa))
  expect_equal(dim(priv$kappa), c(data$n, data$p))
  expect_false(is.na(priv$ZI_cond_mean))

  model2 <- ZINormalBlockUnknownClusters$new(data, 2, control = NB_control(verbose = FALSE))
  priv2  <- model2$.__enclos_env__$private
