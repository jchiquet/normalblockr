...
- [`NormalBlockKnownClusters`](NormalBlockKnownClusters.md) : R6 class
  for a normal-block model with known clustering.
...
- [`fitted(`*`<NormalBlockBase>`*`)`](fitted.NormalBlockBase.md) :
  Extracts fitted values from objects returned by \[NormalBlockBase()\]
  and its variants
- [`generate_normal_block_data()`](generate_normal_block_data.md) :
